...
def main():
    ...
    with tab4:
        ...
        st.markdown('_Otro modelo realizado pero que no se encuentra en esta página..._')
        st.subheader('iv) Embeddings + logistic regression')
        st.markdown('En vez de concatenar los strings de un mismo día, procesamos los tweets con un modelo de transformers en español, ajustado para trabajar con tweets. El modelo genera embeddings a nivel de cada palabra, y mediante el mecanismo de self-attention, combina esos embeddings al nivel de cada Tweet. Para obtener un embedding de cada día, se realizó un mean pooling. Luego estas 256 dimensiones generadas por el embedding fueron tomadas como features para entrenar diversos modelos de clasificación, de los cuales nos quedamos con Logistic Regression')
